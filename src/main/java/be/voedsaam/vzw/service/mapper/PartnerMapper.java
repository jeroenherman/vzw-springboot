...
@Component
public class PartnerMapper extends AbstractMapper<Partner, UserDTO>{
	...
	@Override
	public UserDTO mapToDTO(Partner b) {
		if (b==null)
			return null;
		...
		dto.setStocks(b.getStocks().stream().map(schedule -> schedule.getName()).collect(Collectors.toList()));
		return dto;
	}

	@Override
	public Partner mapToObj(UserDTO d) {
		if (d==null)
			return null;
		User b = new Partner();
